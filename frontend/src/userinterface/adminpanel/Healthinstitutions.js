...
var currentItem = {};
export default class Healthinstitutions extends Component {
  state = {
    drawerOpen: false,
    navPath: 0,
    items: [],
    ...
    deleteDialog: false,
    todelete: "",
    //Create Health Institution parameters
    newcode:'',
    newdescription:''
  };
  ...
  async componentDidMount() {
    ...
    let items = await listhealthinstitutions();
    if (items) {
  ...
  render() {
    return (
      <div>
        ...
        >
          <DialogTitle>Create a Health Institution</DialogTitle>
          <DialogContent>
            <TextField
                autoFocus
                margin="dense"
        ...
        >
          <DialogTitle>Delete Health Institution</DialogTitle>
          <DialogContent>
            <DialogContentText>
  ...
  async removeItem() {
    this.setState({ loading: true });
    let DeletionPromise = await deletehealthinstitution(
      currentItem.pkhealthinstitutionid
    );
  ...
  async createItem() {
    ...
    if (this.state.newcode.toString().length > 1 && this.state.newdescription.toString().length > 1) {
      ...
      let createUserOperation = await createhealthinstitution(objectToSend);
      let items = await listhealthinstitutions();
      if (items) {
