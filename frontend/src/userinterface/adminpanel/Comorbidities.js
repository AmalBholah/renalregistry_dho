...
var currentItem = {};
export default class Comorbidities extends Component {
  state = {
    drawerOpen: false,
    navPath: 0,
    items: [],
  ...
  async componentDidMount() {
    ...
    let items = await listcomorbiditiesmaster();
    if (items) {
  ...
  render() {
    return (
      <div>
        ...
        >
          <DialogTitle>Create a Comorbidity</DialogTitle>
          <DialogContent>
            <TextField
              autoFocus
              margin="dense"
        ...
        >
          <DialogTitle>Delete a Comorbidity</DialogTitle>
          <DialogContent>
            <DialogContentText>
  ...
  async removeItem() {
    this.setState({ loading: true });
    let DeletionPromise = await deletecomorbiditiesmaster(
      currentItem.pkcomorbidityid
    );
  ...
  async createItem() {
    ...
    if (this.state.newname.toString().length > 1) {
      ...
      let createUserOperation = await createcomorbiditiesmaster(objectToSend);
      let items = await listcomorbiditiesmaster();
      if (items) {
