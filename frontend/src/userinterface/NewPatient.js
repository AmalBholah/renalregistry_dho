...
import {
  ...
  readeraedtatable, listhemodialysisunits
} from "../apis/master-lists";

import { registerpatient } from "../apis/patient-registration";
...
export default class NewPatient extends Component {
  state = {
    ...
    modeofdialysis:"Hemodialysis (HD)",
    //STATE ADDITIONS
      ...
      <div>
        <AppBar sx={{ position: "relative" }}>
          <Toolbar>
            ...
            <Button
              id="fade-button"
              onClick={() => this.setState({ navPath: 7 })}
              variant="contained"
              color="secondary"
              startIcon={<ArrowBackIcon />}
            >
              Back to list of patients
            </Button>
          </Toolbar>
        </AppBar>
        <Paper
          ...
          <Box sx={{ mb: 2 }} className={"bottom-centered"}>
            <div>
              <Button
                ...
                style={{ fontWeight: "bold" }}
              >
                {this.state.activeStep === 3 ? "Create Patient" : "Continue"}
              </Button>
              <Button
  ...
  async registerPatient() {
    if (this.state.activeStep == 3) {
      ...
      this.setState({ operationComplete: true, navPath: 7 });
    }
  }
